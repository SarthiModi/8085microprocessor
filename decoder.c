...
int findLabelAddress(char *name){
    for(int i = 0; i < totalLabels; i++){
        if(strcmp(labelDirectory[i].name, name) == 0)
            return labelDirectory[i].address;
    }
...
void decodeInstruction(char *line)
{
    ...
    else if(strcmp(op,"JMP")==0){ sscanf(line,"JMP %s",lbl); cpuState.PC = findLabelAddress(lbl)-1; }
    else if(strcmp(op,"JZ")==0){ sscanf(line,"JZ %s",lbl); if(cpuState.Z) cpuState.PC = findLabelAddress(lbl)-1; }
    else if(strcmp(op,"JNZ")==0){ sscanf(line,"JNZ %s",lbl); if(!cpuState.Z) cpuState.PC = findLabelAddress(lbl)-1; }
    else if(strcmp(op,"JM")==0){ sscanf(line,"JM %s",lbl); if(cpuState.S) cpuState.PC = findLabelAddress(lbl)-1; }
    else if(strcmp(op,"JNC")==0){ sscanf(line,"JNC %s",lbl); if(!cpuState.CY) cpuState.PC = findLabelAddress(lbl)-1; }

    else if(strcmp(op,"PUSH")==0){ sscanf(line,"PUSH %c",&r1); PUSH(r1); }
    else if(strcmp(op,"POP")==0){ sscanf(line,"POP %c",&r1); POP(r1); }
    else if(strcmp(op,"CALL")==0){ sscanf(line,"CALL %s",lbl); CALL(findLabelAddress(lbl)); }
    else if(strcmp(op,"RET")==0){ RET(); }
    else if(strcmp(op,"HLT")==0){ cpuState.PC = totalLines; }
